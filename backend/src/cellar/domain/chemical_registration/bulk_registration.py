    ...
    def __init__(
        ...
        *,
        ...
        if not success and action != BulkRegistrationItemAction.ERROR:
            raise ValidationError("non-error action requires success=True")
    ...
    def _guard_transition(self, target: BulkRegistrationStatus) -> None:
        ...
        if target not in allowed:
            raise ValidationError(
                f"Cannot transition bulk registration status from '{self.status}' to '{target}'"
            )

    def _guard_processing(self) -> None:
        if self.status != BulkRegistrationStatus.PROCESSING:
            raise ValidationError("Can only record results while bulk registration is processing")
