...
_SYNTHESIS_TRANSITIONS: dict[SynthesisStatus, set[SynthesisStatus]] = {
    SynthesisStatus.VIRTUAL: {
        SynthesisStatus.DESIGNED,
        SynthesisStatus.SYNTHESIZED,
        SynthesisStatus.PURCHASED,
    },
...
_LIFECYCLE_TRANSITIONS: dict[LifecycleStage, set[LifecycleStage]] = {
    ...
    LifecycleStage.PRECLINICAL_CANDIDATE: {
        LifecycleStage.DEVELOPMENT_CANDIDATE,
        LifecycleStage.DEPRIORITIZED,
        LifecycleStage.ARCHIVED,
    },
...
class Molecule(AggregateRoot):
    ...
    def advance_synthesis(self, target: SynthesisStatus) -> None:
        ...
        if target not in allowed:
            raise ValidationError(
                f"Cannot transition synthesis_status from '{self.synthesis_status}' to '{target}'"
            )
    ...
    def advance_lifecycle(
        ...
        *,
        ...
        if target not in allowed:
            raise ValidationError(
                f"Cannot transition lifecycle_stage from '{self.lifecycle_stage}' to '{target}'"
            )
        if target in (LifecycleStage.DEPRIORITIZED, LifecycleStage.ARCHIVED) and not reason:
            raise ValidationError(f"Reason is required when transitioning to '{target}'")
    ...
    def validate_mixture_composition(self) -> None:
        ...
        if self.molecule_type == MoleculeType.MIXTURE and len(self.mixture_components) < 2:
            raise ValidationError("Mixture molecules must have at least 2 components")
