...
class InterceptSpec:
    ...
    * ``ABSOLUTE``: an absolute Y value the curve must cross (rare).
    ...
    def __post_init__(self) -> None:
        if self.basis == InterceptBasis.RELATIVE_PERCENT and not (0 < self.level < 100):
...
class DoseResponseConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.activity_threshold is not None and not (0 <= self.activity_threshold <= 100):
            raise ValidationError("DoseResponseConfig activity_threshold must be in [0, 100]")
        if (
        ...
        if self.bottom_constraint is not None and (
            self.bottom_constraint_min is not None or self.bottom_constraint_max is not None
        ):
            raise ValidationError(
        ...
        if (
            ...
            raise ValidationError(
                "DoseResponseConfig bottom_constraint_min must be less than bottom_constraint_max"
            )
        if (
        ...
        if not (0 < self.full_r2_min <= 1):
            raise ValidationError("DoseResponseConfig full_r2_min must be in (0, 1]")
        if not (0 < self.partial_r2_min <= 1):
            raise ValidationError("DoseResponseConfig partial_r2_min must be in (0, 1]")
        ...
        if not self.intercepts:
            default_kind = (
                InterceptKind.IC if self.curve_type == CurveType.IC50 else InterceptKind.EC
            )
            object.__setattr__(
                self,
                "intercepts",
        ...
        for spec in self.intercepts:
            ...
            if key in seen:
                raise ValidationError(f"DoseResponseConfig has duplicate intercept spec {key}")
            seen.add(key)
