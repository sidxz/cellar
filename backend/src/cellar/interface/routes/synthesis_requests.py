...
CreateSynthesisRequestDep = Annotated[
    CreateSynthesisRequest, Depends(_get_use_case(CreateSynthesisRequest))
]
SubmitSynthesisRequestDep = Annotated[
    SubmitSynthesisRequest, Depends(_get_use_case(SubmitSynthesisRequest))
]
ApproveSynthesisRequestDep = Annotated[
    ApproveSynthesisRequest, Depends(_get_use_case(ApproveSynthesisRequest))
]
RejectSynthesisRequestDep = Annotated[
    RejectSynthesisRequest, Depends(_get_use_case(RejectSynthesisRequest))
]
AssignSynthesisRequestDep = Annotated[
    AssignSynthesisRequest, Depends(_get_use_case(AssignSynthesisRequest))
]
...
FulfillSynthesisRequestDep = Annotated[
    FulfillSynthesisRequest, Depends(_get_use_case(FulfillSynthesisRequest))
]
FailSynthesisDep = Annotated[FailSynthesis, Depends(_get_use_case(FailSynthesis))]
CancelSynthesisRequestDep = Annotated[
    CancelSynthesisRequest, Depends(_get_use_case(CancelSynthesisRequest))
]
GetSynthesisRequestDep = Annotated[
    GetSynthesisRequest, Depends(_get_use_case(GetSynthesisRequest))
]
UpdateSynthesisRequestDep = Annotated[
    UpdateSynthesisRequest, Depends(_get_use_case(UpdateSynthesisRequest))
]
DeleteSynthesisRequestDep = Annotated[
    DeleteSynthesisRequest, Depends(_get_use_case(DeleteSynthesisRequest))
]
ListSynthesisRequestsDep = Annotated[
    ListSynthesisRequests, Depends(_get_use_case(ListSynthesisRequests))
]
...
async def get_synthesis_request(
    ...
    result = await uc(
        GetSynthesisRequestQuery(workspace_id=auth.workspace_id, request_id=request_id), auth=auth
    )
    request = result_to_response(result)
    return SynthesisRequestResponse.from_domain(request)
...
async def delete_synthesis_request(
    ...
    result = await uc(
        DeleteSynthesisRequestCommand(workspace_id=auth.workspace_id, request_id=request_id),
        auth=auth,
    )
...
async def approve_synthesis_request(
    ...
    result = await uc(
        ApproveSynthesisRequestCommand(
            workspace_id=auth.workspace_id, request_id=request_id, approved_by=auth.user_id
        ),
        auth=auth,
    )
    request = result_to_response(result)
...
async def start_synthesis(
    ...
    result = await uc(
        StartSynthesisCommand(
            workspace_id=auth.workspace_id,
            request_id=request_id,
            proposed_route_id=body.proposed_route_id,
        ),
        auth=auth,
    )
    request = result_to_response(result)
...
@router.post(
    "/synthesis-requests/{request_id}/flag-infeasible", response_model=SynthesisRequestResponse
)
...
async def fulfill_synthesis_request(
    ...
    result = await uc(
        FulfillSynthesisRequestCommand(
            workspace_id=auth.workspace_id, request_id=request_id, batch_id=body.batch_id
        ),
        auth=auth,
    )
    request = result_to_response(result)
