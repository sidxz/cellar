...
class ShipmentResponse(BaseModel):
    ...
    def from_domain(cls, s: Shipment) -> ShipmentResponse:
        return cls(
            ...
            expected_arrival_date=s.expected_arrival_date.isoformat()
            if s.expected_arrival_date
            else None,
...
MarkShipmentInTransitDep = Annotated[
    MarkShipmentInTransit, Depends(_get_use_case(MarkShipmentInTransit))
]
...
async def delete_shipment(shipment_id: uuid.UUID, auth: AuthDep, uc: DeleteShipmentDep) -> None:
    result = await uc(
        DeleteShipmentCommand(workspace_id=auth.workspace_id, shipment_id=shipment_id), auth=auth
    )
