...
CreateSynthesisRouteDep = Annotated[
    CreateSynthesisRoute, Depends(_get_use_case(CreateSynthesisRoute))
]
GetSynthesisRouteDep = Annotated[GetSynthesisRoute, Depends(_get_use_case(GetSynthesisRoute))]
ListSynthesisRoutesByMoleculeDep = Annotated[
    ListSynthesisRoutesByMolecule, Depends(_get_use_case(ListSynthesisRoutesByMolecule))
]
...
ValidateSynthesisRouteDep = Annotated[
    ValidateSynthesisRoute, Depends(_get_use_case(ValidateSynthesisRoute))
]
SetPreferredRouteDep = Annotated[SetPreferredRoute, Depends(_get_use_case(SetPreferredRoute))]
UpdateSynthesisRouteDep = Annotated[
    UpdateSynthesisRoute, Depends(_get_use_case(UpdateSynthesisRoute))
]
DeleteSynthesisRouteDep = Annotated[
    DeleteSynthesisRoute, Depends(_get_use_case(DeleteSynthesisRoute))
]
RemoveReactionStepDep = Annotated[RemoveReactionStep, Depends(_get_use_case(RemoveReactionStep))]
DeprecateSynthesisRouteDep = Annotated[
    DeprecateSynthesisRoute, Depends(_get_use_case(DeprecateSynthesisRoute))
]
...
async def list_synthesis_routes(
    ...
    result = await uc(
        ListSynthesisRoutesByMoleculeQuery(
            workspace_id=auth.workspace_id, target_molecule_id=molecule_id
        ),
        auth=auth,
    )
...
async def get_synthesis_route(
    ...
    result = await uc(
        GetSynthesisRouteQuery(workspace_id=auth.workspace_id, route_id=route_id), auth=auth
    )
    route = result_to_response(result)
    return SynthesisRouteResponse.from_domain(route)
...
async def delete_synthesis_route(
    ...
    result = await uc(
        DeleteSynthesisRouteCommand(workspace_id=auth.workspace_id, route_id=route_id), auth=auth
    )
...
async def remove_reaction_step(
    ...
    result = await uc(
        RemoveReactionStepCommand(
            workspace_id=auth.workspace_id, route_id=route_id, step_id=step_id
        ),
        auth=auth,
    )
    route = result_to_response(result)
