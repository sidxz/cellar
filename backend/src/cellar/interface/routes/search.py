...
_MetricSpec = Annotated[_MetricTanimoto | _MetricTversky, Field(discriminator="kind")]
...
class _SimilarityMatch(BaseModel):
    ...
    def _check(self) -> "_SimilarityMatch":
        ...
        if (
            ...
            raise ValueError("explicit algorithm requires metric + threshold (or use mode)")
        return self
...
_structure_adapter: TypeAdapter[_ExactMatch | _SubstructureMatch | _SimilarityMatch] = TypeAdapter(
    _StructureClause
)
...
def _attach_score(m: Molecule, scores: dict[uuid.UUID, float] | None) -> MoleculeResponse:
