...
class GetMergeImpact:
    ...
    async def __call__(
        ...
        target_row = await self._reader.get_molecule_summary(ws, input.target_molecule_id)
        ...
        if counts.batch_count:
            categories.append(
                MergeImpactCategory(name="batches", label="Batches", count=counts.batch_count)
            )
        ...
        # Sample requests
        total_sr = counts.active_sample_request_count + counts.terminal_sample_request_count
        if total_sr:
            categories.append(
                MergeImpactCategory(
