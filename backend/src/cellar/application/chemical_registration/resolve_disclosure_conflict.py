...
from cellar.domain.chemical_registration.enums import DisclosureStatus, MergeReason
...
class ResolveDisclosureConflict:
    ...
    async def __call__(
        ...
        async with self._uow:
            ...
            molecule = await self._molecule_repo.find_by_id_in_workspace(
                input.workspace_id, dr.molecule_id
            )
            if molecule is None:
                return Failure(NotFoundError("Molecule", str(dr.molecule_id)))

            if dr.status != DisclosureStatus.CONFLICT:
                return Failure(
                    ValidationError(f"Disclosure is in '{dr.status.value}' status, not 'conflict'")
                )
    ...
    async def _handle_accept_as_new(
        ...
        if isinstance(process_result, Failure):
            return Failure(
                ValidationError(f"Structure processing failed: {process_result.failure().message}")
            )
    ...
    async def _handle_accept_merge(
        ...
        async with self._uow:
            target = await self._molecule_repo.find_by_inchi_key(input.workspace_id, inchi_key)
            if target is None:
                return Failure(
                    ConflictError("No existing molecule with matching InChIKey found for merge")
                )
