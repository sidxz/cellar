...
class PreviewShipmentImport:
    ...
    async def _resolve_compound(
        ...
        mol = await self._molecule_repo.find_by_registration_number(workspace_id, raw.upper())
        if mol:
            resolved.compound_id = str(mol.id)
            display_name = mol.name or ""
            reg = mol.registration_number.value if mol.registration_number else ""
            resolved.compound_display = f"{reg} -- {display_name}" if display_name else reg
            if raw != (reg or raw):
                resolved.corrections.append(
                    FieldCorrection(
                        field="compound",
        ...
        if mol is None:
            mol = await self._molecule_repo.find_by_identifier(workspace_id, raw.lower())
        if mol:
            resolved.compound_id = str(mol.id)
            reg = mol.registration_number.value if mol.registration_number else ""
            resolved.compound_display = f"{reg} -- {mol.name or ''}"
