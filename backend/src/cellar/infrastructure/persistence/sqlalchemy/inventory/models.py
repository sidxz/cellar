...
class SampleModel(Base, EntityModelMixin, WorkspaceIdMixin, VersionMixin):
    ...
    location_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("storage_locations.id"))
    freeze_thaw_count: Mapped[int] = mapped_column(Integer, nullable=False, server_default="0")
...
class StorageLocationModel(Base, EntityModelMixin, WorkspaceIdMixin, VersionMixin):
    ...
    parent_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, ForeignKey("storage_locations.id"))
...
class RegisteredPlateModel(Base, EntityModelMixin, WorkspaceIdMixin, VersionMixin):
    ...
    status: Mapped[str] = mapped_column(String(20), nullable=False, server_default="registered")
...
class ImportTemplateModel(Base, EntityModelMixin, WorkspaceIdMixin):
    ...
    __table_args__ = (Index("ix_import_template_ws", "workspace_id"),)
