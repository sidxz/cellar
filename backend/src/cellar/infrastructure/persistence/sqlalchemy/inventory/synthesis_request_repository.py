...
class SQLAlchemySynthesisRequestRepository(
    ...
    def _to_domain(self, model: SynthesisRequestModel) -> SynthesisRequest:
        ...
        return SynthesisRequest(
            ...
            feasibility_status=FeasibilityStatus(model.feasibility_status)
            if model.feasibility_status
            else None,
    ...
    def _to_model(self, aggregate: SynthesisRequest) -> SynthesisRequestModel:
        return SynthesisRequestModel(
            ...
            target_smiles=aggregate.target_structure.smiles
            if aggregate.target_structure
            else None,
            target_inchi=aggregate.target_structure.inchi if aggregate.target_structure else None,
            target_inchi_key=aggregate.target_structure.inchi_key
            if aggregate.target_structure
            else None,
            ...
            assignment_type=aggregate.assignment.assignment_type.value
            ...
            feasibility_status=aggregate.feasibility_status.value
            if aggregate.feasibility_status
            else None,
    ...
    def _update_model(self, model: SynthesisRequestModel, aggregate: SynthesisRequest) -> None:
        model.target_smiles = (
            aggregate.target_structure.smiles if aggregate.target_structure else None
        )
        model.target_inchi = (
            aggregate.target_structure.inchi if aggregate.target_structure else None
        )
        model.target_inchi_key = (
            aggregate.target_structure.inchi_key if aggregate.target_structure else None
        )
        ...
        model.assignment_type = (
            aggregate.assignment.assignment_type.value if aggregate.assignment else None
        )
        model.assigned_to = aggregate.assignment.assigned_to if aggregate.assignment else None
        model.assigned_org_id = (
            aggregate.assignment.assigned_org_id if aggregate.assignment else None
        )
        model.proposed_route_id = aggregate.proposed_route_id
        model.feasibility_notes = aggregate.feasibility_notes
        model.feasibility_status = (
            aggregate.feasibility_status.value if aggregate.feasibility_status else None
        )
        ...
        model.actual_cost_unit = (
            aggregate.actual_cost.unit.value if aggregate.actual_cost else None
        )
