...
class SQLAlchemyAuditRepository:
    ...
    async def save_with_session(self, operation: AuditOperation, session: AsyncSession) -> None:
    ...
    def _to_operation_model(op: AuditOperation) -> AuditOperationModel:
        ...
        model.entries = [SQLAlchemyAuditRepository._to_entry_model(e) for e in op.entries]
        if op.signature is not None:
            model.signature = SQLAlchemyAuditRepository._to_signature_model(op.signature)
        return model
