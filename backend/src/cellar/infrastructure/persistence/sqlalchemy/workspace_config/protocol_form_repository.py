...
class SQLAlchemyProtocolFormRepository(SQLAlchemyRepository[ProtocolForm, ProtocolFormModel]):
    ...
    @staticmethod
    def _ontology_defaults_to_json(
        defaults: list[ProtocolFormOntologyDefault],
    ) -> list[dict] | None:
        if not defaults:
            return None
        from dataclasses import asdict
