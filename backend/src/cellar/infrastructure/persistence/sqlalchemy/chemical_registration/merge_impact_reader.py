...
from cellar.application.chemical_registration.merge_impact_reader import (
    MergeImpactCounts,
    MoleculeSummaryRow,
)
        ...
        async with self._session_factory() as session:
            ...
            readout_count = (
                await session.execute(
                    ...
                    .where(
                        ReadoutDataModel.molecule_id == src, ReadoutDataModel.workspace_id == ws
                    )
                )
            ).scalar_one()
            ...
            curve_count = (
                await session.execute(
                    ...
                    .where(
                        DoseResponseCurveModel.molecule_id == src,
                        DoseResponseCurveModel.workspace_id == ws,
                    )
                )
            ).scalar_one()
            ...
            flag_count = (
                await session.execute(
                    ...
                    .where(
                        CompoundFlagModel.molecule_id == src, CompoundFlagModel.workspace_id == ws
                    )
                )
            ).scalar_one()
            ...
            route_count = (
                await session.execute(
                    ...
                    .where(
                        SynthesisRouteModel.target_molecule_id == src,
                        SynthesisRouteModel.workspace_id == ws,
                    )
                )
            ).scalar_one()
