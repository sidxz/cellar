...
class SQLAlchemyMoleculeRepository(SQLAlchemyRepository[Molecule, MoleculeModel]):
    ...
    def _set_optional_fields(model: MoleculeModel, aggregate: Molecule) -> None:
        ...
        model.stereochemistry = (
            aggregate.stereochemistry.value if aggregate.stereochemistry else None
        )
    ...
    @staticmethod
    def _ident_to_model(
        ident: MoleculeIdentifier, workspace_id: uuid.UUID
    ) -> MoleculeIdentifierModel:
    ...
    async def find_by_ids(self, workspace_id: uuid.UUID, ids: list[uuid.UUID]) -> list[Molecule]:
    ...
    async def find_active(
        ...
        *,
        ...
        if search_term:
            ...
            identifier_subq = select(MoleculeIdentifierModel.molecule_id).where(
                MoleculeIdentifierModel.workspace_id == workspace_id,
        ...
        if project_ids is not None:
            ...
            unscoped_subq = select(MoleculeModel.id).where(
                MoleculeModel.workspace_id == workspace_id,
    ...
    async def remove_from_project(
        ...
        stmt = molecule_projects.delete().where(
            ...
            molecule_projects.c.molecule_id.in_(
                select(MoleculeModel.id).where(MoleculeModel.workspace_id == workspace_id)
            ),
        )
        await self._session.execute(stmt)
