...
class SQLAlchemyBulkRegistrationRepository(
    ...
    async def find_by_workflow_id_in_workspace(
        ...
        stmt = select(BulkRegistrationModel).where(
    ...
    async def insert_items(self, items: list[BulkRegistrationItem]) -> None:
        ...
        stmt = (
            pg_insert(BulkRegistrationItemModel)
            .values(rows)
            .on_conflict_do_nothing(index_elements=["bulk_registration_id", "row_index"])
        )
    ...
    def _update_model(self, model: BulkRegistrationModel, aggregate: BulkRegistration) -> None:
