...
class SQLAlchemyPlateMapReader:
    ...
    async def get_plate_map(
        ...
        async with self._session_factory() as session:
            ...
            dose_unit = (await session.execute(dose_unit_stmt)).scalar_one_or_none() or "uM"
                ...
                for well in sorted(plate_wells[plate.id], key=lambda w: (w.row, w.column)):
                    ...
                    well_entries.append(
                        WellMapEntry(
                            ...
                            batch_number=(
                                batch_numbers.get(well.batch_id) if well.batch_id else None
                            ),
                            molecule_id=molecule_id,
                            molecule_name=(mol_names.get(molecule_id) if molecule_id else None),
                            synonyms=tuple(
                                mol_synonyms.get(molecule_id, []) if molecule_id else ()
                            ),
                            smiles=(mol_smiles.get(molecule_id) if molecule_id else None),
                            dose=well.dose,
                        )
                    )
                plates_data.append(
...
def _summarize(wells: list[WellMapEntry]) -> PlateMapSummary:
    ...
    control_wells = sum(1 for w in wells if w.well_type != _SAMPLE and w.well_type != "")

    # Compounds = unique molecules represented in sample wells.
    sample_molecules = [
        w.molecule_id for w in wells if w.well_type == _SAMPLE and w.molecule_id is not None
    ]
    ...
    if per_compound_conc:
        concentrations_per_compound = max(len(s) for s in per_compound_conc.values())
    else:
