...
class SQLAlchemyProtocolStatsReader:
    ...
    async def get_stats_data(
        ...
        async with self._session_factory() as session:
            ...
            compound_count = (await session.execute(compound_count_stmt)).scalar_one()
            ...
            latest_run_row = (await session.execute(latest_run_stmt)).one_or_none()
            ...
            if latest_run_row is not None:
                ...
                plate_count_stmt = (
                    select(func.count()).select_from(PlateModel).where(PlateModel.run_id == run_id)
                )
                plate_count = (await session.execute(plate_count_stmt)).scalar_one()
                ...
                run_compound_count = (await session.execute(run_compound_stmt)).scalar_one()
