...
class CollectionModel(Base, EntityModelMixin, WorkspaceIdMixin, VersionMixin):
    ...
    __table_args__ = (UniqueConstraint("workspace_id", "name", name="uq_collection_ws_name"),)
...
class CollectionMoleculeModel(Base):
    ...
    __table_args__ = (Index("ix_collection_molecules_molecule_id", "molecule_id"),)
...
class SavedSearchModel(Base, EntityModelMixin, WorkspaceIdMixin, VersionMixin):
    ...
    __table_args__ = (Index("ix_saved_searches_ws_creator", "workspace_id", "created_by"),)
...
class CampaignResultModel(Base, EntityModelMixin):
    ...
    molecule_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), nullable=False)
...
class CampaignMeasurementModel(Base, EntityModelMixin):
    ...
    test_concentration_value: Mapped[float | None] = mapped_column(Float, nullable=True)
    test_concentration_unit: Mapped[str | None] = mapped_column(String(32), nullable=True)
