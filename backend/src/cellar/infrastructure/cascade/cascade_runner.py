...
from cellar.application.admin.cascade_service import CascadeExecutionError
...
class CascadeRunner:
    ...
    async def execute(
        ...
        *,
        ...
        for table_name, ids in reversed(delete_ops):
            sa_table = Base.metadata.tables[table_name]
            await self._session.execute(delete(sa_table).where(sa_table.c.id.in_(ids)))
        ...
        await self._session.execute(delete(root_table).where(root_where))
        return entries
    ...
    async def _collect(
        ...
        *,
        ...
        rows = (await self._session.execute(sa_table.select().where(snap_where))).mappings().all()
        ...
        for rule in get_rules_for_parent(table):
            ...
            if "workspace_id" in child_sa.c:
                child_fk_where = child_fk_where & (child_sa.c["workspace_id"] == workspace_id)
            ...
            if "id" not in child_sa.c:
                if rule.action == CascadeAction.CASCADE:
                    ...
                    await self._session.execute(delete(child_sa).where(child_fk_where))
            ...
            child_ids = [
                row.id
                for row in (
                    await self._session.execute(select(child_sa.c.id).where(child_fk_where))
                ).all()
            ]
            if not child_ids:
                continue
