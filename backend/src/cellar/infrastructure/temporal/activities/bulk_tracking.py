...
class BulkTrackingActivities:
    ...
    async def persist_chunk_items(self, input: PersistChunkItemsInput) -> None:
        ...
        async with uow:
            ...
            mol_ids = {uuid.UUID(it["molecule_id"]) for it in input.items if it.get("molecule_id")}
            molecules = await mol_repo.find_by_ids(ws_id, list(mol_ids)) if mol_ids else []
            ...
            for raw in input.items:
                ...
                molecule_id = uuid.UUID(molecule_id_str) if molecule_id_str else None
    ...
    @activity.defn
    async def update_cdd_plate_import_progress(
        self, input: UpdateCddPlateImportProgressInput
    ) -> None:
