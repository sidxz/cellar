...
Tautomer policy: we do NOT apply RDKit's ``TautomerEnumerator.Canonicalize``.
Its default rule set tautomerizes alpha-H positions through C=O, which
silently destroys stereo on amino acids, sugars, profens, beta-lactams,
and most compounds with a stereocenter adjacent to an enolizable
carbonyl — collapsing L/D enantiomers to the same InChIKey. Standard
InChI's mobile-H layer is sufficient for the tautomer-equivalence cases
we want to merge (pyridones, imidazoles, tetrazoles, guanines), and is
the approach ChEMBL, PubChem, and CAS use in production. Tautomer-aware
*search* is handled at query time via the cartridge's xqmol / @>>
operator, not at registration time.
"""
...
class StructureStandardizer:
    ...
    Pipeline: parse SMILES -> standardize -> get parent -> generate canonical
    SMILES, InChI, InChIKey, molfile. See module docstring for the rationale
    behind not running an additional tautomer canonicalization step.
    """
    ...
    def standardize(self, raw_smiles: str) -> Result[StandardizedMolecule, DomainError]:
        ...
        try:
            ...
            # 3. Generate representations directly from the parent.
            # NOTE: no tautomer canonicalization here — see module docstring.
            # Standard InChI's mobile-H layer handles tautomer dedup correctly.
            # The chembl pipeline returns a mol whose ring info isn't always
            # initialized; downstream stereo APIs need it, so prime it here.
            Chem.FastFindRings(parent_mol)
            canonical_smiles = MolToSmiles(parent_mol)
            cxsmiles = MolToCXSmiles(parent_mol)
            molfile = MolToMolBlock(parent_mol)

            inchi = Chem.MolToInchi(parent_mol)
            if inchi is None:
                return Failure(StandardizationError(raw_smiles, "Failed to generate InChI"))

            inchi_key = Chem.InchiToInchiKey(inchi)
            if inchi_key is None:
                return Failure(StandardizationError(raw_smiles, "Failed to generate InChIKey"))
    ...
    def check_molecule(self, mol: object) -> QCResult:
        ...
        try:
            ...
            structlog.get_logger(__name__).warning(
                "QC check failed — returning zero penalty",
                exc_info=True,
            )
