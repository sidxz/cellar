...
class PlateRegistrationActivities:
    ...
    async def process_plate_chunk(self, input: PlateChunkInput) -> PlateChunkOutput:
        ...
        for plate_item in input.items:
            ...
            try:
                ...
                logger.exception(
                    "Error processing plate cdd_plate_id=%s", plate.cdd_plate_id,
                )
    ...
    async def _process_single_plate(
        ...
        if isinstance(reg_result, Failure):
            logger.warning(
                "Failed to register plate %s: %s", barcode, reg_result.failure(),
            )
            output.plates_error += 1
            return None
        ...
        if plate.wells:
            well_map = await self._resolve_wells(
                session_factory, ws_id, input.cdd_vault_id, plate.wells, output,
            )
            if well_map:
                await map_wells_uc(
                    MapWellsCommand(
                ...
                for cdd_bid in cdd_batch_ids:
                    stmt = (
                        ...
                        .where(
                            ...
                            cast(BatchModel.custom_fields["cdd_batch_id"].astext, String) == str(cdd_bid),
                        )
                        .limit(1)
                    )
