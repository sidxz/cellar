...
Bracket syntax: chem-vault readout names can contain spaces (e.g. "Raw AU"),
...
def _expand_brackets(
    ...
    expanded["__chem_vault_alias_map__"] = aliases  # type: ignore[assignment]
...
def _swap_aliases_in_undefined(
    undefined: list[str], aliases: dict[str, str]
) -> list[str]:
    ...
    return [
        f"[{reverse[v]}]" if v in reverse else v for v in undefined
    ]
...
class AstevalFormulaEvaluator:
    ...
    def run(
        ...
        aliases: dict[str, str] = expanded.pop(
            "__chem_vault_alias_map__", {}  # type: ignore[arg-type]
        )
        ...
        if interp.error:
            ...
            raise FormulaValidationError(
                formula, _unswap_alias_in_message(first.msg, aliases)
            )
