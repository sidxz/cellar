...
class CddPlateImportModel(Base, EntityModelMixin, WorkspaceIdMixin, VersionMixin):
    ...
    plates_registered: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default="0"
    )
    plates_duplicate: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default="0"
    )
    plates_error: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default="0"
    )
    wells_mapped: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default="0"
    )
    wells_unresolved: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default="0"
    )
    ...
    __table_args__ = (
        Index("ix_cdd_plate_import_ws_status", "workspace_id", "status"),
    )
