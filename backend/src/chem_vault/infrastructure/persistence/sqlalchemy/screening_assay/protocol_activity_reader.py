        ...
        async with self._session_factory() as session:
            ...
            drc_mols = (
                select(
                    ...
                    func.count(distinct(DoseResponseCurveModel.run_id)).label(
                        "run_count"
                    ),
                    func.max(RunModel.run_date).label("last_tested"),
                )
            ...
            numeric_stmt = (
                ...
                .group_by(
                    ReadoutDataModel.molecule_id, ReadoutDefinitionModel.name
                )
            )
            ...
            dr_agg_stmt = (
                select(
                    ...
                    func.exp(
                        func.avg(func.ln(DoseResponseCurveModel.fitted_value))
                    ).label("geo_mean"),
                )
                .join(RunModel, DoseResponseCurveModel.run_id == RunModel.id)
                .where(
            ...
            ranked_sub = (
                ...
                .join(RunModel, DoseResponseCurveModel.run_id == RunModel.id)
                .outerjoin(
                    BatchModel, DoseResponseCurveModel.batch_id == BatchModel.id
                )
                .where(
                    DoseResponseCurveModel.protocol_id == pid,
                    RunModel.workspace_id == ws,
            ...
            raw_best_params_rows = (
                await session.execute(best_params_stmt)
            ).all()
