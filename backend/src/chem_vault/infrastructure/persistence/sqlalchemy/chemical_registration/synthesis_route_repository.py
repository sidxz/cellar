...
class SQLAlchemySynthesisRouteRepository(
    ...
    def _step_to_domain(sm: ReactionStepModel) -> ReactionStep:
        conditions = None
        cond_fields = [
            sm.condition_solvent, sm.condition_temperature, sm.condition_pressure,
            sm.condition_catalyst, sm.condition_atmosphere, sm.condition_time,
        ...
        outcome = None
        outcome_fields = [
            sm.outcome_yield_percent, sm.outcome_crude_yield_percent,
            sm.outcome_purity_percent, sm.outcome_actual_scale_value,
            sm.outcome_purification_method,
        ]
        if any(f is not None for f in outcome_fields):
            actual_scale = None
            if sm.outcome_actual_scale_value is not None and sm.outcome_actual_scale_unit is not None:
                actual_scale = Amount(
        ...
        reagents = [
            ReactionReagent(
                ...
                amount=Amount(value=r["amount"]["value"], unit=AmountUnit(r["amount"]["unit"])) if r.get("amount") else None,
    ...
    def _step_to_model(step: ReactionStep) -> ReactionStepModel:
        reagent_dicts = [
            {
                ...
                "supplier": r.supplier,
                "amount": {"value": r.amount.value, "unit": r.amount.unit.value} if r.amount else None,
        ...
        return ReactionStepModel(
            ...
            condition_additional=step.conditions.additional_conditions if step.conditions else None,
            ...
            outcome_actual_scale_value=step.outcome.actual_scale.value if step.outcome and step.outcome.actual_scale else None,
            outcome_actual_scale_unit=step.outcome.actual_scale.unit.value if step.outcome and step.outcome.actual_scale else None,
