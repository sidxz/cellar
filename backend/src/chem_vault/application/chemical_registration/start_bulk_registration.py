...
class StartBulkRegistration:
    ...
    async def __call__(
        ...
        try:
            ...
            return Failure(
                ValidationError(f"Unsupported file format: {input.file_format!r}")
            )
        ...
        try:
            ...
            return Success(
                StartBulkRegistrationResult(mode="async", workflow_id=workflow_id)
            )
        ...
        return Success(
            StartBulkRegistrationResult(mode="sync", sync_outcome=sync_result.unwrap())
        )
