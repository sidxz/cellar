...
class ParsePlateMapFile:
    ...
    def _parse_well_level(
        ...
        return Success(
            ParsedPlateMap(assignments=assignments, unresolved=[], row_count=row_count)
        )
...
class SetUpRunPlate:
    ...
    async def __call__(
        ...
        async with self._uow:
            ...
            ref_to_mol: dict[str, uuid.UUID] = {
                r.ref.value: r.molecule_id for r in resolved
            }
            ...
            for mol_id in ref_to_mol.values():
                if mol_id not in mol_to_batch:
                    batches = await self._batch_repo.find_by_molecule(
                        input.workspace_id, mol_id
                    )
                    if batches:
    ...
    @staticmethod
    def _parse_position(
        pos: str, index: int, conc_series: list[float]
    ) -> tuple[str, int]:
