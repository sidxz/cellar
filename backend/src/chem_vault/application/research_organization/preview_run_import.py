...
def _apply_selection_rule(
    candidates: list[ResolvedCandidate], rule: SelectionRule
) -> _Picked | None:
    """Apply a selection rule to a pre-filtered candidate list.

    Returns None for ND (empty candidates or MANUAL_PICK).
    """
    if not candidates:
        return None
    ...
    async def _execute(
            ...
            channels_meta.append(
                {
                    ...
                    "hit_threshold": (
                        cfg.hit_threshold.to_dict() if cfg.hit_threshold else None
                    ),
            ...
            for molecule_id, candidates in candidates_by_mol.items():
                ...
                if picked is None:
                    ...
                    cell = {
                        ...
                        "source_run_id": (
                            str(picked.source_run_id)
                            if picked.source_run_id
                            else None
                        ),
                        "source_run_date": (
                        ...
                        "contributing_run_ids": [
                            str(rid) for rid in picked.contributing_run_ids
                        ],
                    }
        ...
        for mid, row in rows_by_molecule.items():
            ...
            if not active_cells:
                ...
                row["is_hit"] = (
                    any(hits) if q.filter_mode == "any" else all(hits)
                )
        ...
        return Success(
            {
                "summary": {
                    "runs": len(q.run_ids),
                    "channels_new": sum(
                        1 for c in channels_meta if c["source"] == "new"
                    ),
                    "channels_reused": sum(
                        1 for c in channels_meta if c["source"] == "reused"
                    ),
