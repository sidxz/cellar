    ...
    async def __call__(
        ...
        async with self._uow:
            ...
            if campaign.status != CampaignStatus.DRAFT:
                return Failure(
                    ValidationError(
                        f"Cannot add results: campaign is {campaign.status.value}"
                    )
                )
            ...
            for mol_id, ch_cells in cells_by_mol.items():
                ...
                if not active_hits:
                    ...
                    mol_is_hit[mol_id] = (
                        any(active_hits)
                        if input.filter_mode == "any"
                        else all(active_hits)
                    )
            ...
            for mol_id in new_mols:
                ...
                for ch_id, cell in cells_by_mol[mol_id].items():
                    ...
                    if (
                        ...
                        or (cell.picked.run_date or datetime.max.date())
                        < earliest_date
                    ):
...
def _build_measurement(
    *,
    ...
    picked = cell.picked
    from chem_vault.domain.research_organization.enums import HitCall
