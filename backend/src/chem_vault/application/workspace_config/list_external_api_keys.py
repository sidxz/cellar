"""ListExternalApiKeys query — list API key metadata for a workspace."""

from __future__ import annotations

import uuid
from dataclasses import dataclass

from returns.result import Result, Success

from chem_vault.application.auth import AuthContext, require_workspace_role
from chem_vault.application.shared.query import Query
from chem_vault.application.shared.unit_of_work import UnitOfWork
from chem_vault.domain.shared.errors import DomainError
from chem_vault.domain.workspace_config.external_api_key import ExternalApiKey
from chem_vault.domain.workspace_config.repository import ExternalApiKeyRepository


@dataclass(frozen=True, kw_only=True)
class ListExternalApiKeysQuery(Query):
    workspace_id: uuid.UUID


class ListExternalApiKeys:
    def __init__(self, uow: UnitOfWork, repo: ExternalApiKeyRepository) -> None:
        self._uow = uow
        self._repo = repo

    async def __call__(
        self, input: ListExternalApiKeysQuery, auth: AuthContext | None = None
    ) -> Result[list[ExternalApiKey], DomainError]:
        require_workspace_role(auth, "viewer")
        async with self._uow:
            results = await self._repo.find_by_workspace(input.workspace_id)
            return Success(results)
