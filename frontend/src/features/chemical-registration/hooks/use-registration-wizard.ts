...
export const useRegistrationWizard = create<RegistrationWizardState>()((set, _get) => ({
  // Initial state
  mode: null,
  currentStep: 0,
  ...
  // Input updates
  updateSingleInput: (patch) => set((s) => ({ singleInput: { ...s.singleInput, ...patch } })),
  updateBulkInput: (patch) => set((s) => ({ bulkInput: { ...s.bulkInput, ...patch } })),
