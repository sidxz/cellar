...
describe("deriveChannelHitDefaults", () => {
  ...
  it("carries a single numeric criterion verbatim", () => {
    const d = deriveChannelHitDefaults([num("IC50", "lt", 100)], {
      name: "IC50",
  ...
  it("pairs gt+lt into `between` (the bug the user caught)", () => {
    const d = deriveChannelHitDefaults([num("IC50", "gt", 10), num("IC50", "lt", 100)], {
      name: "IC50",
      data_type: "dose_response",
    });
    expect(d).toMatchObject({
  ...
  it("pairs gte+lte the same way", () => {
    const d = deriveChannelHitDefaults([num("IC50", "lte", 50), num("IC50", "gte", 5)], {
      name: "IC50",
      data_type: "dose_response",
    });
  ...
  it("only matches numeric criteria for the named readout", () => {
    const d = deriveChannelHitDefaults([num("EC50", "gt", 10), num("IC50", "lt", 100)], {
      name: "IC50",
      data_type: "dose_response",
    });
  ...
  it("carries the Curve Class filter for dose-response readouts", () => {
    const d = deriveChannelHitDefaults([num("IC50", "lt", 100), curveClass("full", "partial")], {
      name: "IC50",
      data_type: "dose_response",
    });
  ...
  it("ignores Curve Class for non-DR readouts (the chip filter would dangle otherwise)", () => {
    const d = deriveChannelHitDefaults([curveClass("full")], {
      name: "Raw Data",
      data_type: "numeric",
    });
    expect(d.allowed_curve_classes).toEqual([]);
  ...
  it("carries intercept_key from a single-criterion threshold", () => {
    const d = deriveChannelHitDefaults([num("Resazurin", "lt", 50, EC90)], {
      name: "Resazurin",
      data_type: "dose_response",
    });
    expect(d.hit_operator).toBe("lt");
  ...
  it("curve-class criteria are not affected by the intercept filter", () => {
    ...
    const d = deriveChannelHitDefaults(
      [num("Resazurin", "lt", 50, EC90), curveClass("full", "partial")],
      { name: "Resazurin", data_type: "dose_response" },
      EC90,
    );
    expect(d.hit_operator).toBe("lt");
