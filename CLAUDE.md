...
**Branch:** `prot-2`, N commits ahead of `e807dd03` (use `git rev-list --count e807dd03..HEAD`) (the merged `fe2` HEAD). Browser-smoke for Surfaces #1–#7 passed 2026-05-14; commits #16 (Option A — multi-intercept channels), #17 (mirror-protocol bulk-create), and #18 (curve-expand DRY) all need fresh smoke. Nothing pushed. Dev DB migrated to head (`035_cc_intercept_key`); commit #18 needs no migration but the rebuild_campaign_curve_snapshots.py script needs `--include-closed` approval to retroactively bring existing closed campaigns up to the new snapshot shape.
...
18. **`570f67b6` — `feat(campaign): expand-dialog renders via shared DoseResponseChart`** (closes the curve-expand DRY follow-up from the original handoff)
    - **The chemist's gripe.** Click a curve thumbnail in the campaign post-close grid → expand dialog opens with a custom strip showing the headline value (with an `IC50` hardcode pre-bandaid; `EC50`/`EC90`-derived after the bandaid) and renders the chart via the lightweight `<DoseResponseFigure>`. The same compound's curve in the search compound-detail sheet renders via `<DoseResponseChart isInteractive={false}>` and looks materially different — `Card`-styled container, intercept secondary chips (`EC90` chip when the protocol declares one), CI strip, fit-quality-warning badges. Same biological data, two surfaces, different look.
    - **Root cause.** The campaign's `curve_snapshot` JSONB (migration 031) carried `fitted_value`, `top`, `bottom`, `hill_slope`, `r_squared`, `curve_class`, `raw_data`. `<DoseResponseChart>`'s SummaryCard keys its richer rendering off four fields the snapshot didn't have: `curve_type`, `intercept_values`, `confidence_interval_low/high`, `fit_quality_warnings`. Without them, the chart degraded to "no chip strip / no CI strip / legacy curve_type label" — which is exactly what the campaign dialog had been doing manually all along.
    - **Backend** (`channel_resolution.py`): `ResolvedCandidate` gains four new fields (`curve_type`, `curve_confidence_interval_low/high`, `curve_fit_quality_warnings`); `intercept_values` was already there since Surface #7. Both SQL pull paths (`_fetch_curve_candidates` + `fetch_candidates_for_runs` curve branch in `channel_resolution_query.py`) extend their `select(...)` and `ResolvedCandidate(...)` construction. `_build_curve_snapshot` writes the four new keys when present; legacy keys unchanged. **No DB migration** — `curve_snapshot` is JSONB; new keys land additively on freshly-imported measurements.
    - **Frontend** (`screening-assay/components/dose-response-figure.tsx`): `CurveSnapshot` interface gains the four new optional fields. New `screen-campaign/lib/snapshot-adapter.ts::snapshotToDoseResponseCurve(snap, ctx)` widens the snapshot to the chart's expected `DoseResponseCurve` shape with placeholder UUIDs for FK fields the chart never reads (matches the search-side `adaptCurve(CurveDetail, ...)` co-location pattern). Legacy snapshots without the new fields degrade gracefully — chart's SummaryCard falls back to `curve_type = "ic50"` and shows no chip strip / no CI strip / no warning badges (i.e., the same render as the pre-fix dialog), instead of crashing.
    - **`curve-expand-dialog.tsx`** rewritten from ~100 LOC (custom strip + `CurveClassBadge` + `<DoseResponseFigure>`) to ~50 LOC (just dialog shell + `<DoseResponseChart curves={[adapted]} isInteractive={false}>`). Drops the previous hardcoded `IC50` bandaid and the per-curve metric strip duplication entirely. The `curveSnapshotFromMeasurement` helper in `results-grid.tsx` pulls the new fields from the live FK fallback path too (when a draft campaign hasn't yet frozen its snapshot).
    - **Backfill** (`backend/scripts/rebuild_campaign_curve_snapshots.py`): walks every `campaign_measurement` with non-null `source_curve_id`, re-reads the source DR curve, and rewrites the JSONB via the same `_build_curve_snapshot` helper. Idempotent + resumable; `--workspace-id` / `--campaign-id` / `--dry-run` flags. **Closed campaigns require an explicit `--include-closed` flag** that bypasses the `reject_locked_campaign_write` trigger via `SET LOCAL session_replication_role = 'replica'` per UPDATE transaction. Aggregate-rule rows (MEAN_ACROSS_RUNS / GEOMETRIC_MEAN, `source_curve_id IS NULL`) are counted but not rebuilt — chemist must Refresh-from-sources via the campaign UI for those. Dry-run on dev: 44 rebuildable measurements + 0 aggregate. **Live backfill not yet executed** — needs explicit user approval since `--include-closed` touches a 21 CFR Part 11 compliance trigger.
    - **Tests.** 1 new BE integration test (`test_curve_candidate_carries_chart_fields_into_snapshot`) seeds a curve with all four fields populated, asserts the SQL pull lifts them onto `ResolvedCandidate`, and asserts `_build_curve_snapshot` threads them into the dict. 4 new FE adapter tests (post-fix snapshot threading, legacy graceful degrade, FK placeholder check, num_points-from-raw_data).
    - 9 files / +762 / −58. 218 BE research_org tests green (217 → 218 = +1). 156 FE tests green (152 → 156 = +4). `pnpm exec tsc --noEmit` clean.
...
**Verification:**
- Backend: research_org subset post-#16/#17/#18 = **218 passed** (211 → 217 → 218 = +6 mirror + +1 chart-snapshot). Commit #16 added `test_multi_intercept_channels_with_display_only_second_intercept` and updated 4 existing resolver tests; commit #17 added 6 mirror-use-case tests; commit #18 added `test_curve_candidate_carries_chart_fields_into_snapshot`. No domain layer breakage. Migration 035 applied to dev DB; commit #18 needs no migration.
- Frontend: **156 tests green** (152 → 156 = +4 new adapter tests in commit #18). `pnpm exec tsc --noEmit` clean throughout.
- Refit script ran successfully on dev DB; orval regen committed.
- **Browser smoke: PASSED on 2026-05-14** against the live `Mtb_WCA_mc2-7000_Resazurin` protocol — Surfaces #1–#7 walk cleanly. Commits #12 (unified Export menu), #14, #15, **#16 (Option A)**, **#17 (mirror-protocol)**, **#18 (curve-expand DRY)** still need fresh browser smokes.
...
3. **Live smoke commit #18** on the same `Mtb_WCA_mc2-7000_Resazurin` campaign:
   - Click any compound's curve thumbnail in an EC50 column. The expand dialog should render via `<DoseResponseChart>` (Card chrome, intercept chip strip with EC50 + EC90 chips, CI strip if data has it) — bit-identical to clicking the same compound's curve in the search compound-detail sheet's DR card.
   - **For pre-#18 campaigns** (the existing closed `Mtb_WCA_mc2-7000_Resazurin` you're testing): the snapshot was frozen before commit #18 added the four chart fields, so the dialog will show the legacy "no chip strip / no CI strip / curve_type-fallback label" render even after this commit. Two ways to fix:
     a. **Run the backfill script** (`uv run python backend/scripts/rebuild_campaign_curve_snapshots.py --include-closed --campaign-id <uuid>`). The `--include-closed` flag bypasses the closed-campaign immutability trigger; the script only touches `curve_snapshot` JSONB (display metadata), not values or hit calls. **Requires explicit user approval** — the classifier blocks this command until allow-listed.
     b. **Create a fresh campaign** of the same protocol via add-from-runs. New imports get the new fields automatically; the chart renders correctly without any backfill.
4. **Smoke commits #12 (unified Export menu) + #14 (add-from-runs split) + #15 (channel-key collision)** if not already done — independent of Option A / Mirror / DRY, safe to ship.
5. **Push** — `prot-2` is N commits ahead of `e807dd03` (use `git rev-list --count e807dd03..HEAD`), nothing pushed. After all live smokes pass, push the branch and open a PR against `main`.
...
- Backend curve_snapshot: `application/research_organization/channel_resolution.py::_build_curve_snapshot` — single helper that freezes a `ResolvedCandidate`'s curve shape into the JSONB stored on every `CampaignMeasurement`. Adding a field for the campaign expand-dialog → add it to `ResolvedCandidate`, populate from both SQL paths in `channel_resolution_query.py`, and write into the dict here. Then extend `CurveSnapshot` interface in `dose-response-figure.tsx` so the chart picks it up.
- FE snapshot adapter: `screen-campaign/lib/snapshot-adapter.ts::snapshotToDoseResponseCurve` — only place the campaign's `CurveSnapshot` is widened to the chart's `DoseResponseCurve` shape. Co-located template for if a future surface needs the same widening.
...
**Open follow-up: Campaign curve-expand DRY** — **CLOSED in commit #18.** The campaign expand dialog now renders via `<DoseResponseChart isInteractive={false}>` matching protocol-runs and search. New-import measurements automatically carry the four extra snapshot fields (curve_type / intercept_values / CI / warnings); historical closed campaigns need the backfill script run with `--include-closed` to bring their existing snapshots up to date. The closed brief at `docs/superpowers/specs/2026-05-14-campaign-curve-expand-dry-handoff.md` is the reference for what was shipped.
