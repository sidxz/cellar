...
- **Channel-key collision + eval_value bugs** (commit #14 hardening) — *partially done in commit #15.* Fixes work when channels have intercept-keyed thresholds (backend unit test verifies). **The chemist's actual workflow — one channel with threshold + one display-only channel — still produces 0 hits + identical cell values.** Root cause: `intercept_key` is bound to `hit_threshold`; when the chemist has no threshold, intercept identity is dropped before the wire. **Full architectural fix deferred to a fresh session.** See `docs/superpowers/specs/2026-05-14-multi-intercept-channels-handoff.md` for the design re-evaluation brief, three options (A: top-level `intercept_key` on `CampaignChannel`; B: synthesized marker hit_threshold; C: revert), and a per-file diagnostic anchor list.

**How to resume:**
1. **Read the multi-intercept channels brief first**: `docs/superpowers/specs/2026-05-14-multi-intercept-channels-handoff.md`. It contains the chemist scenario, what commits #13–#15 each shipped, why none of them fully resolve the workflow, three architectural options, an open-questions list to discuss with the user, and per-file diagnostic anchors. The user explicitly asked for a re-evaluation in a fresh session — start there before touching code.
2. **Decide path forward with the user** (Option A: top-level `intercept_key` on `CampaignChannel` with alembic migration ≈13 files; Option B: synthesized marker hit_threshold — rejected as long-term but tactically viable; Option C: revert commits #13/#14/#15).
3. **Smoke commit #12 (unified Export menu)** if not already done — open the search page, run a search returning DR results, confirm ResultsToolbar lost its standalone "Export" button (just Add to Collection / Customize Report / Save Search), confirm a new "Export" dropdown sits above the grid with three options (Excel / CSV / SDF), and exercise each download. Independent of the multi-intercept work; safe to ship.
4. **Push** — `prot-2` is N commits ahead of `e807dd03` (use `git rev-list --count e807dd03..HEAD`), nothing pushed. Whether to push depends on the multi-intercept decision: pushing the current state preserves the partial fix and the new test; reverting would create a fourth commit on top.
