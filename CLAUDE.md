...
**Verification:**
- Backend: 2422 → 2438 → 456 → 2424 → 2452 green at each commit (final sweep after Surface #7). No backend touched in commits #12, #13, or #14.
...
- **Browser smoke: PASSED on 2026-05-14** against the live `Mtb_WCA_mc2-7000_Resazurin` protocol — Surfaces #1–#7 walk cleanly. Commit #13 (channel intercept picker) partially smoked — the "Kinase" campaign showed EC50-only on import, which motivated commit #14 (per-intercept channel split in add-from-runs). Commit #12 (unified Export menu) and commit #14 still need fresh browser smokes.
