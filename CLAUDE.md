...
**Verification:**
- Backend: 2422 → 2438 → 456 → 2424 → 2452 green at each commit (final sweep after Surface #7). No backend touched in commits #12 or #13.
- Frontend: 106 → 115 → 122 → 127 → 138 → 147 tests green. `pnpm exec tsc --noEmit` clean throughout.
- Refit script ran successfully on dev DB; orval regen committed.
- **Browser smoke: PASSED on 2026-05-14** against the live `Mtb_WCA_mc2-7000_Resazurin` protocol — Surfaces #1–#7 walk cleanly. The unified Export menu (commit #12) and channel intercept picker (commit #13) ship untested in browser; need follow-up smokes.
...
2. **Smoke commit #13 (channel intercept picker)** — needs a campaign whose protocol has a 2-intercept DR readout (e.g. `Mtb_WCA_mc2-7000_Resazurin` with EC50 + EC90 — the same protocol used for the post-033 smoke). Walkthrough:
   - **Carry-forward.** Set protocol-level recommended_hit_criteria to "Resazurin EC90 < 50" (Surface #7's dialog). Open the campaign, add a channel for Resazurin from DR-curve source. Expect the channel popover's hit-threshold row to show `[EC90 ▼] [< less than ▼] [50]` and the caption "Hit if Resazurin EC90 < 50 uM". Save.
   - **Display.** Verify channels-section chip reads `hit if EC90 < 50`.
   - **Edit.** Re-open the channel popover. EC90 still selected. Toggle to EC50 (primary). Save. Chip now reads `hit if < 50` (primary stays implicit). Re-open — EC50 selected. Save without changes. Backend should persist `intercept_key: null`.
   - **Legacy.** A campaign created before Surface #7 has `intercept_key` absent on the channel. Open its popover. Picker shows primary selected (fallback). No data corruption.
   - **Add-from-runs.** Use the "Add from runs" dialog. Channels auto-create with `intercept_key` matching the protocol recommendation. Open each to verify.
   - **Non-DR.** A `readout_data` channel: picker not rendered (correct — no curve = no intercept). Save: `intercept_key: null`.
3. **Push** — `prot-2` is N commits ahead of `e807dd03` (use `git rev-list --count e807dd03..HEAD`), nothing pushed. `git push -u origin prot-2` (or merge to `main` if both smokes pass).
