...
> **Unit B — server atomic swap, ✅ DONE** (2026-06-15, branch `design-7`). The SAR table **and** heatmap now read from the Unit-A server endpoints: the table is an AG-Grid Infinite Row Model over `POST /sar/decomposition/{run}/rows` (server pagination + sort + **filter**, per-row `activity_snapshot`, response-level `activity_reference`); the heatmap reads server-aggregated cells from `POST /sar/decomposition/{run}/heatmap`; `SarView` orchestrates `useDecompositionRun` + `useActivityProjection` (job-poll) and **prefers `collectionId`** — so SAR is correct over the **full collection** of any size. This substantially resolves **item #1** below (full-collection coverage); the client-side compute (`use-sar-activity`, `use-rgroup-decomposition`, `rgroup-heatmap-grid`) is deleted, so the `useSarActivity` bullet in **#4** is obsolete. Plan: `docs/superpowers/plans/2026-06-15-sar-unit-b-frontend-swap.md`.
>
> **Deferred to Unit C:**
> - **Playwright E2E** — the repo has no active E2E harness (all `tests/e2e/*.spec.ts` are `.TODO` stubs: no auth fixture / no seeding). A SAR-workbench smoke (core → decompose → activity → table sort/filter → heatmap → curve-expand) needs that harness first. Unit-B logic is otherwise covered: BE reader+API tests, FE hook tests, component pure-helper tests, and a `SarView` orchestration smoke.
> - **Component render-flow tests** — `rgroup-table.test.tsx` / `rgroup-heatmap.test.tsx` were slimmed to **pure-helper** coverage (the data flow is now server-driven via hooks that carry their own tests; `fragmentDisplay` cell rendering is covered by `sar-fragment-label.test.ts`). Full render-flow assertions belong in the E2E above.
> - Plus the plan's closing list: server-side "Save all N matched → collection" (`POST …/save-collection`), loading/empty/cancel-state copy polish, perf indexes (`rgroups->>'Rn'` expression + join indexes), the domain-model deviation note in `docs/domain-model/04-sar-analysis.md`, and the GitHub board update. Item **#3** (extract shared `pickReference`/`potencyShade`/`snapshotFromActivity` out of `rgroup-table.tsx`) still stands — the heatmap rewrite continues to import them from that sibling component.
