...
- **Post-S12 addenda (2026-08-25, user-requested, bounded):** (a) tree polish `32a42ae9` after a live side-by-side with the legacy dashboard — root rendered as circle + name, type-coloured name pills, 13 px rows, subtree plate counts, `nodeSize` 380×300, zoom 0.8. (b) **Group details page replaces the tree side panel** (legacy "activity log report" layout): `GET /api/v1/plate-groups/{id}` → `PlateGroupDetailResponse{group, plate_count, subtree_plate_count, plate_format, ancestors[], children[]}` (`GetPlateGroup`, hidden == 404; `bdb53142`); FE route `/inventory/plate-groups/[id]` → `plate-group-page.tsx` on `DetailShell` — breadcrumb from ancestors, header actions Request loan / Add plates / Add child / Edit / Move / Delete, Details + Child groups left, Activity (`CommentFeed`) + Plates right; `plate-group-details.tsx` deleted; the tree's Details link and pill click navigate to the page (`24201b55`).
