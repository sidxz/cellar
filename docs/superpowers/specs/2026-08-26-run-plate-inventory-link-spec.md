...
## S15 sync note (2026-08-26) — shipped reality vs. §2–§8

- Shipped in `ba9a6f1b` (backend + regenerated client) and `572a1e42` (frontend); plan `docs/superpowers/plans/2026-08-26-s15-run-plate-inventory-link.md`, seven tasks in three waves, one whole-branch review (**APPROVE**, no findings).
- Two amendments made during the build and recorded above: a locked run answers **409 `ConflictError`** (the existing `Run._guard_not_locked` convention), and runs have no name — the runs-for-plate row/response carry **`run_date`** (`Run {date}` in the UI).
- Beyond the text: the new FK needed a Tier-2 **cascade rule** (`rules_screening_assay.py`, `SET_NULL`) — `test_fk_coverage.py` enforces that every FK to an admin-deletable table is categorised, and the DB `ondelete` alone does not satisfy it; `tests/api/conftest.py` now mounts the `plate_setup` router (it never had been — no API test had exercised `/plate-setup` or `/plate-map`); the auto-link lives in a separate async pass (`_autolink_new_plates`) called only from `ImportRunFile._execute`, so the pure conflict scan and the preview path are untouched; `ImportRunFile`'s plate repo + visibility deps are optional (`None` = no auto-link) so existing constructors keep working; the "Used in runs" row key is `run_id + plate_number` because one physical plate can appear as several plates of one run.
- Migration 069 applied to the local dev DB (saclab-dev): 7 run plates, 4 with file names, **0 back-filled** — none of the dev runs' plate names match a migrated TAMU plate (expected for that data).
- Suites: backend unit 3109 passed (2 pre-existing failures: WeasyPrint libs, order-dependent FK-coverage — see backlog), 152 API/integration tests in the touched areas passed; frontend **1113 passed**, tsc and biome clean. Browser-verified on saclab-dev: Plate Map tab → Link plate → `005261` → `005261 · SAC1-001-5261 · Unlink` + toast; plate page shows `Used in runs — NadD-Sumo dose response · Run Jun 5, 2026 · Plate 1 · Draft`; Unlink restores `Link plate` (test link removed afterwards).
- Residuals (unchanged from §2/§9): the plate-map read exposes a linked plate's id/barcode/label to anyone who can see the run; auto-link never fires for plate-setup-created plates (no name); label matches are exact and case-sensitive.
