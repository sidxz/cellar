...
| D4 | Run scope for resolution (ask 7) | **Revised at review 2026-09-11.** The campaign records its **seed runs** explicitly (`Campaign.seed_runs: [{run_id, protocol_id}]`, appended by every add-from-runs call including refresh-existing; migration 079 backfills from row `RunRef`s joined to the run's protocol). A `RunRef` records only the run that won a pick, so a union of RunRefs was wrong (a three-run mean collapsed to one run). Scope is **per channel protocol**: a channel resolves within the seed runs of its own protocol; a protocol with no seed runs (a mirrored counter-screen) resolves protocol-wide. Per-channel opt-out `resolve_from_all_runs` stays. Rows added manually or from a collection in a run-seeded campaign are restricted too. |
...
- `Campaign.seed_runs: list[SeedRun(run_id, protocol_id)]` with `record_seed_runs(...)` (draft only, de-duplicated, ordered) and `seed_run_ids_for(protocol_id)`; JSONB `campaign.seed_runs`, migration 079 with a backfill from row `RunRef`s. `add_results_from_runs` records every input run on every call, including the refresh-existing path. A campaign seeded only from another campaign, a collection or by hand has no seed runs and resolves protocol-wide until its first add-from-runs, after which it narrows to those runs for that protocol. (Superseded the union-of-`RunRef` design after the whole-branch review; see D4.)
...
- `ChannelResolver.resolve(..., run_ids=None)` passes through. Module helper `resolution_run_ids(campaign, channel)`: `None` when the channel opts out or the campaign has no seed runs for the channel's protocol; otherwise those run ids (D4). Computed once per channel at each call site.
