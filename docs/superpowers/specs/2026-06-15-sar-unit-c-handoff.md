...
### 2. ✅ DONE (2026-06-16) — Perf (evidence-first)  *(small)*
**Done:** commits `6a177e8d` (resolver UUID batch — kills the N+1: one `find_by_ids` not N, the real save-all win) + `f7fafcd2` (EXPLAIN probe + evidence). **No migration added** — the proposed scoped-join / activity-join indexes are already the composite PKs (`rgroup_assignments (run_id, molecule_id)`, `sar_activity_values (projection_id, molecule_id)`); EXPLAIN with `enable_seqscan=off` confirms index scans throughout, no seq scan on the SAR tables (evidence: `docs/backlog/sar-rows-explain-evidence.md`). GIN-on-`rgroups` not justified (run-scoped filter). Spec+plan: `docs/superpowers/{specs,plans}/2026-06-16-sar-unit-c-item2-perf*.md`.
**Original note (superseded by the evidence above):** The `/rows` filter/sort hits `rgroups->>'Rn'` and the activity LEFT JOIN every page. Add a migration:
...
### 3. ✅ DONE (2026-06-16) — Functional-state + honest-label polish  *(small)*
**Done:** commits `a51852f9` (useDecompositionRun cancel/isCancelled/runAgain) → `9467776a` (useActivityProjection, symmetric) → `0600837a` (SarView six-state taxonomy: inline Cancel on both progress lines, neutral cancelled + Run again, surfaced activity-projection failure, 0-match empty panel with toggle hidden) → `040955a7` (a cancel hides the whole results region — no double-banner). Both hooks now expose `cancel()`/`isCancelled`/`runAgain()`; `getError` no longer folds cancelled into error; `runAgain()` bumps a query-key nonce (cache is READY-only). "no core picked" is owned by the core picker's existing prompt. Spec+plan: `docs/superpowers/{specs,plans}/2026-06-16-sar-unit-c-items345-polish*.md`.
**Now (original):** Loading / empty / **cancel** states + copy across `sar-view`/`rgroup-table`/`rgroup-heatmap`: wire the projection/run **cancel** endpoints (`…/jobs/{id}/cancel`) to a cancel affordance; distinguish "no core picked" vs "decomposing" vs "0 matched" vs "failed"; the counts banner is already honest ("M matched of N"). Heatmap already shows the "top 30 of N" truncation note.

### 4. ✅ DONE (2026-06-16) — Domain-model deviation note  *(tiny; docs only)*
**Done:** commit `e5ee8ed9` — added an "Async-job / read-model aggregates (Part 1b/2 additions)" section to `docs/domain-model/04-sar-analysis.md` documenting `RGroupDecompositionRun` + `SarActivityProjection` (lifecycle pending→running→ready/failed/cancelled, membership/core/channel hashes as READY-only cache identity, RGroupAssignment/ActivityScalar read models, relationship to fingerprint/Markush). (The file was newly git-tracked — joins its already-tracked sibling `05-research-organization.md`.)
**Now (original):** `docs/domain-model/04-sar-analysis.md` does **not** mention `RGroupDecompositionRun` or `SarActivityProjection` (added as async-job/read-model aggregates in Part 1b/2). Add a short section documenting them (job lifecycle: pending→running→ready/failed/cancelled; membership/channel hashes for cache identity; relationship to the existing fingerprint/Markush aggregates).

### 5. ✅ DONE (2026-06-16) — Extract shared SAR activity-display helpers  *(small; followups item #3)*
**Done:** commits `0cffe7f7` (extract `activityValueToCurveSnapshot` → `research-organization/lib/activity-curve-snapshot.ts`, where `ActivityValue` lives — DRYs the duplicate in `dose-response-cell.tsx`) + `544de25d` (extract `pickReference`/`potencyShade` → `sar-analysis/lib/sar-activity-display.ts`; rewire `rgroup-table` + `rgroup-heatmap` — the heatmap no longer imports from the sibling component). Pure, behavior-preserving; helper tests moved with the code. Deviation from the original note below: the curve-snapshot mapper lives in `research-organization/lib` (not `sar-analysis/lib`) to keep the dependency arrow `sar-analysis → research-organization` and avoid a feature cycle.
**Now (original):** `rgroup-heatmap.tsx` imports `pickReference`/`potencyShade`/`snapshotFromActivity` from the sibling **component** `rgroup-table.tsx` (a smell). Extract to `features/sar-analysis/lib/sar-activity-display.ts` (pure + tests), import in both; also DRY `snapshotFromActivity` against the duplicate mapping in `research-organization/.../dose-response-cell.tsx`.
