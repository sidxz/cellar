...
### 1. Server-side "Save all N matched → collection"  *(small–moderate; highest user value)*
**Now:** the table's "Save as collection" only saves the **selected/loaded** rows (`SarView` → `SaveSelectionDialog` → create-collection + bulk-add by id). Over a full collection the chemist can't save *all* matches (or all matches under the current filter) without scrolling every page.
