...
## S17 sync note (2026-08-26) — shipped reality vs. §2–§8

- Shipped in `45eac2f1` (backend + client), `019c38de` (frontend), `1bbff217` (review fix: the demo-data shipment loader still used `sample_id` and would have aborted seeding at stage 19). Plan `docs/superpowers/plans/2026-08-26-s17-shipment-container.md`, three tasks in three waves, one whole-branch review (REQUEST CHANGES for that loader → fixed; everything else verified).
- Beyond the text: item labels come through the `ShipmentsReader` (`item_labels`) rather than the repos — `SampleRepository` has no `find_by_ids` and a sample's label is its batch number on `batches`; `ShipmentLink` carries `carrier` (the card needs it); `AddShipmentItemCommand.item` nests `ShipmentItemInput` (one resolver for create and add); the write path honours the **borrowed carve-out** — a borrower may record shipping a loaned plate back (records only, so the write-narrowing rule in `plate_visibility.py` gets a documented exception); `IGNORED_FKS` gained `shipments.loan_id → plate_loans` (SET NULL; `plate_loans` is no admin tier); `Shipment.update_details` (not `update`) carries the `loan_id` sentinel and keeps its "only while preparing" rule; `tests/api/conftest.py` now mounts the `samples` and `shipments` routers (neither ever was — there were no shipment API tests before S17); FE `types/shipment.ts` aliases the generated bodies (its hand-rolled `sample_id` input was exactly the drift CLAUDE.md warns about); the pre-existing `AddItemDialog` on the shipment page was rewired to resolve a barcode instead of posting it as `sample_id`.
- Known, left as is: the FE never edits `loan_id` after creation (backend `PATCH` supports set/clear; a "carries loan" editor is a follow-up if asked); `AddShipmentItem` lets the domain `ValidationError` propagate (422 via the global handler, pinned by a test) while `UpdateShipment` wraps it — consistent outcome, inconsistent style; the shipment page's **Delete** button did not open a confirmation under browser automation (pre-existing control, not investigated — the test record was removed directly).
- Migration 071 applied to the local dev DB (3 legacy shipments → `outbound`, items → `sample`). Suites: backend unit 3154 passed (2 known pre-existing failures), 125 API in the touched areas; frontend **1138 passed**, tsc/biome clean. Browser-verified on saclab-dev: New Shipment → Inbound relabels the org field "From Organization" → barcodes `005261`, `SMP-CIP-001`, `nope-123` resolve to a plate row (no amount), a sample row (amount input) and a red "Unknown barcode" left in the box → created → list `Inbound · BioSynth CRO · Preparing · 2` → detail `Inbound · Shipment from BioSynth CRO`, items Plate 005261 (—) / Sample SMP-CIP-001 (5 mg) with links → plate page and sample page **Shipments** cards `← BioSynth CRO · Preparing · created Aug 26, 2026`. Test shipment removed afterwards.
