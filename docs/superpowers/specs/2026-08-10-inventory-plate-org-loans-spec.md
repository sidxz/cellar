...
8. **First live `cellar:approve_loan` check_action: CONFIRMED WORKING 2026-08-14** — the runtime rig stubs auth (Sentinel's /authz/resolve requires a live human IdP session), so this was verified manually: the operator signed in as the granted non-admin editor and approved a loan in the UI — the first live RoleClient round-trip succeeded against deployed Sentinel.
...
## S6 sync note (2026-08-14) — shipped reality vs. this spec §12

S6 (legacy MySQL → Cellar migration) shipped via subagent-driven development — plan `docs/superpowers/plans/2026-08-14-s6-legacy-plate-tracker-migration.md`, one script `backend/scripts/migrate_legacy_plate_tracker.py` + unit/integration tests (28/28 green against a real testcontainer Postgres). Per-task reviews + a whole-branch review found **zero Critical and zero data-integrity defects**; every Important finding was fixed in-flight (see below). Issue sidxz/cellar#71.

**Do-first fix (bigger than expected):** the `docs/backlog/alembic-env-*` fix turned out to be a **5-table** hazard, not 2 — `alembic/env.py`'s model-import block was also missing `export_job_model`, `personalization.models`, and `sar_activity_projection_models`, so autogenerate would have proposed `DROP TABLE` on `export_jobs`, `favorites`, and the SAR projection tables too. Completed the block; verified autogenerate at head 064 emits zero `op.drop_table`. Residual raw-SQL-DDL / mixin / timestamp drift documented in that backlog file — **never blindly apply autogenerate**.

**Recorded deviations (legacy reality ≠ §12's assumptions, all deliberate):**
1. **`plate_role` has no `MASTER_TWIN`** — the actual values are {MASTER, SCREENING, HIT_COLLECTION, VENDOR}. `MASTER_TWIN` is a `set_type`, not a plate role. §12.4's `MASTER_TWIN→replicate` PlateType row is dropped; the four real roles map MASTER→mother, SCREENING→assay, HIT_COLLECTION→cherry_pick, VENDOR→compound_storage.
2. **`plate_status` has a third value `AVAIL` (373 plates)** unmapped by §12.4 → **AVAIL→stored** (operator-confirmed; bucketed with Active; the target enum only distinguishes stored/depleted/disposed). Inactive→depleted **+ the `legacy:inactive` tag is applied automatically** (key=`legacy`, value=`inactive`), not left as a manual step.
3. **`TRANSACTION_PLATE` is a current-state table (PK `plate_id`)** holding only the ~203 rows of the 15 OPEN transactions — closed loans' plate membership is gone from it. Closed history is **not** migrated (§12.6); `ACTIVITY_LOG`-based reconstruction is explicitly out of scope.
4. **No MySQL driver existed** (`asyncpg` only) → added `pymysql` (pure-Python, script-only, removable post-cutover); `--legacy-dsn` points at a live legacy MySQL (or a restored dump for the dry-run).
5. **No service-key email→user lookup exists in Sentinel** → requester resolution is legacy `account.UIN → email` → operator-supplied `--user-map` CSV; unmatched → reported and that loan skipped (never fabricated).
6. **Migrated loans are same-org** (owner_org_id == borrower_org_id == internal org — legacy has no org model) and **self-approved** (`approved_by = requested_by`, since legacy `authorized_by` is NULL on open transactions); they bypass the `cellar:approve_loan` check by construction, so no grant is needed for the migration.
7. **The NULL-owner ownership backfill (S2-deferred) runs in-script** (one bulk SQL `UPDATE … WHERE owner_org_id IS NULL`, version-bumped), not as an alembic data migration — the internal org id is a runtime arg, not a hardcodable constant.
8. **All writes go through aggregates/repos via one `AsyncUnitOfWork`** (the one raw-SQL write is the backfill). `uow.commit()` returns events without dispatching them, so the import fires no audit/notification side-effects. The whole migration is one atomic transaction → `--dry-run` is a trivial rollback, and idempotent re-run handles interruption.

**Fixed during review (would-have-been bugs):** apply_loan never advanced `APPROVED`-target items (from the defensive `COUT_WSCAN` mapping) and double-loaned already-active plates on partial overlap (Critical+Important, Task 6); `report_dir` wasn't created before writing the CSVs → operator crash-after-commit (Task 7); `summary["groups_created"]` reported tree size not created-count, lying on a re-run (whole-branch review). All fixed with tests.

**Operator notes / cutover runbook:** the runbook is the script's module docstring. **Run the MANDATORY `--dry-run` against a restored `~/workspace/legacy/intranet/sacnet_prod.sql` first** — review the summary + `unmatched_plates.csv` / `unresolved_requesters.csv`, resolve gaps (barcodes/cdd, add emails to `--user-map`), then the real run. Args: `--legacy-dsn --workspace-id --internal-org-id --cdd-vault-id --actor-id --user-map --report-dir [--dry-run]`. §11's plate-detail CODE128 render remains UNSCHEDULED (deviation #9 from S5, still parked). Deferred minor findings: `docs/backlog/s6-legacy-migration-deferred-minors.md` (none block; one to confirm at dry-run — a plate already DEPLETED/DISPOSED in Cellar while legacy says Active hard-aborts the atomic run).
